...
// Copyright (c) 2013 The Babylonian developers
...
uint256 hashGenesisBlock("0x0000047a51b6a7edd5d2c37213d0f4da827572c69920ce6099a1e21a92ea5904");
static const unsigned int timeGenesisBlock = 1385607600;
...
const string strMessageMagic = "Babylonian Signed Message:\n";
...
static const int64 nTargetTimespan = 100 * 60; // approx 8 hours
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    else if(nHeight < 1441)
    {
        nSubsidy = 60 * COIN;
    }
    else if(nHeight < 46801)
    {
        nSubsidy = 40 * COIN;
    }

    // Subsidy is cut in half every 1 month
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "hello bbl";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 1386388712;
		
        if (fTestNet)
        {
            block.nTime    = 1385607600;
            block.nNonce   = 1386388712;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x6e19a6772ab142f1b9b7266b58bcc3ef22a9d82684eff66b0defbc3144dd55de"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BabylonianMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BabylonianMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("BabylonianMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Babylonian-miner");
    ...
    try { loop {
        ...
        printf("Running BabylonianMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("BabylonianMiner terminated\n");
        throw;
    }
}
